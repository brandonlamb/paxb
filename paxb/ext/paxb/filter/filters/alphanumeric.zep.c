...
ZEPHIR_INIT_CLASS(PAXB_Filter_Filters_AlphaNumeric) {
	...
	ZEPHIR_REGISTER_CLASS_EX(PAXB\\Filter\\Filters, AlphaNumeric, paxb, filter_filters_alphanumeric, paxb_filter_filters_regexp_ce, paxb_filter_filters_alphanumeric_method_entry, 0);
...
PHP_METHOD(PAXB_Filter_Filters_AlphaNumeric, apply) {
	...
	ZEPHIR_INIT_VAR(_1);
	ZEPHIR_CONCAT_SVS(_1, "/[^\\p{L}\\p{N}", whitespaceChar, "]/u");
	zephir_update_property_zval(rule, SL("unicodePattern"), _1 TSRMLS_CC);
	ZEPHIR_INIT_VAR(_2);
	ZEPHIR_CONCAT_SVS(_2, "/[^a-zA-Z0-9", whitespaceChar, "]/");
	zephir_update_property_zval(rule, SL("pattern"), _2 TSRMLS_CC);
	ZEPHIR_RETURN_CALL_PARENT(paxb_filter_filters_alphanumeric_ce, this_ptr, "apply", &_3, rule, value);
